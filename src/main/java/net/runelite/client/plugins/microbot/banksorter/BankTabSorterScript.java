...
public class BankTabSorterScript extends Script {
    ...
    private static String classifyItem(BankSortItem item) {
        ...
        if (name.contains("teleport tab") || name.contains("teleport scroll") || name.endsWith(" teleport"))
            return "Teleportation";
        if (name.contains("potion") && !name.contains("prayer book page"))
            return "Potions"; // Exclude "prayer book page set" for example.
        // Basic food check - might need expansion
        if (name.contains("shark") || name.contains("monkfish") || name.contains("karambwan") || name.contains("anglerfish") || name.contains("cake") || name.contains("pie") || name.contains("stew") || name.contains("pizza") || name.contains("bread"))
            return "Food";
        if (processedName.endsWith(" rune")) return "Runes";
        if (name.contains("dart") || name.contains("arrow") || name.contains("bolts") || name.contains("javelin") || name.contains("throwing axe") || name.contains("knife") || name.contains("chinchompa") || name.endsWith(" shot") || name.endsWith(" shell"))
            return "Ammunition";
        ...
        if (name.contains(" axe") && !name.contains("battleaxe") && !name.contains("greataxe") && !name.contains("throwing axe"))
            return "Tool-Axe"; // woodcutting axe
        if (name.contains("harpoon") || name.contains("fishing rod") || name.contains("fly fishing rod") || name.contains("lobster pot") || name.contains("small fishing net") || name.contains("big fishing net") || name.contains("fishing bait") || name.contains("feather"))
            return "Tool-Fishing";
        if (name.contains("tinderbox") || name.contains("chisel") || name.contains("hammer") || name.contains("spade") || name.contains("rake") || name.contains("seed dibber") || name.contains("secateurs") || name.contains("watering can") || name.contains("glassblowing pipe"))
            return "Tool-Other";


        // Weapons (Melee, Ranged, Magic) - Order by commonality/exclusivity
        // Magic weapons often contain "staff" or "wand"
        if (name.contains("staff") || name.contains("wand") || name.contains("trident") || name.contains("sceptre") || name.contains("crozier") || name.contains("book of") || name.contains("ancient sceptre"))
            return "Weapon-Magic";
        // Ranged weapons
        if (name.contains("shortbow") || name.contains("longbow") || name.contains("comp bow") || name.contains("crossbow") || name.contains("c'bow") || name.contains("ballista") || name.contains("blowpipe") || name.contains("crystal bow") || name.contains("dark bow"))
            return "Weapon-Ranged";
        // Melee (broad category, ensure it's checked after more specific weapon types)
        if (name.contains("scimitar") || name.contains("sword") || name.contains("longsword") || name.contains("dagger") || name.contains("mace") || name.contains("warhammer") || name.contains("battleaxe") || name.contains("halberd") || name.contains("spear") || name.contains("hasta") || name.contains("whip") || name.contains("bludgeon") || name.contains("rapier") || name.contains("greataxe") || name.contains("greatsword") || name.contains("maul") || name.contains("flail") || name.contains("claws"))
            return "Weapon-Melee";
        ...
        if (name.contains("helm") || name.contains("coif") || name.contains("hood") || name.contains("mask") || name.contains("circlet") || name.contains("sallet") || name.contains("med helm") || name.contains("full helm"))
        ...
        if (name.contains("platebody") || name.contains("chainbody") || name.contains("body") && !name.contains("body rune") || name.contains("chestplate") || name.contains("hauberk") || name.contains("robe top") || name.contains("apron") || name.contains("d'hide body") || name.contains("leather body"))
            return "Armour-Body";
        if (name.contains("platelegs") || name.contains("plateskirt") || name.contains("legs") && !name.contains("pegasian") /*pegasian legs*/ || name.contains("skirt") && !name.contains("chainskirt") || name.contains("chaps") || name.contains("tassets") || name.contains("robe bottom") || name.contains("d'hide chaps"))
            return "Armour-Legs";
        if (name.contains("kiteshield") || name.contains("sq shield") || name.contains("shield") && !name.contains("dragonfire ward") || name.contains("defender") || name.contains("toktz-ket-xil") || name.contains("book of") || name.contains("ward") && !name.contains("dwarf weed"))
            return "Armour-Shield";
        if (name.contains("gloves") || name.contains("gauntlets") || name.contains("vambraces") || name.contains("bracers") || name.contains("d'hide vamb"))
        ...
        if (name.contains("ring") && !name.contains("slayer") && !name.contains("ring of dueling"))
            return "Armour-Ring"; // Exclude utility rings if they go elsewhere

        // Skilling Resources
        if (name.contains("uncut") && (GEM_LEVELS.containsKey(processedName) || GEM_LEVELS.containsKey(name.replace("uncut ", ""))))
            return "Skilling-Resource-Gem"; // Uncut gems
        if (GEM_LEVELS.containsKey(processedName) || GEM_LEVELS.containsKey(name))
            return "Skilling-Resource-Gem"; // Cut gems
        ...
        if (SEED_LEVELS_FARMING.containsKey(processedName) || name.endsWith(" seed") || name.endsWith(" sapling"))
            return "Skilling-Resource-Seed";
        if (name.startsWith("raw ") || name.startsWith("cooked ")) return "Skilling-Resource-Fish"; // Crude fish check
        if (name.contains("bone") || name.contains("ashes") || name.contains("ensouled head") || name.contains("hide") || name.contains("fleece") || name.contains("essence") || name.contains("vial of water") || name.contains("vial") && name.length() < 10 || name.contains("eye of newt") || name.contains("limpwurt root") || name.contains("snape grass") || name.contains("bird nest") || name.contains("bucket of"))
            return "Skilling-Resource-Other";


        if (name.contains("clue scroll") || name.contains("reward casket") || name.contains("master scroll book"))
            return "Clue-Scrolls";
        if (name.contains("quest item placeholder") || name.contains("key") && !name.contains("crystal key"))
            return "Quest-Items"; // Generic key check
        if (name.contains("event") || name.contains("holiday") || name.contains("cosmetic") || name.contains("santa hat") || name.contains("partyhat"))
            return "Cosmetic/Holiday";
    ...
    private static String getResourceBucket(String baseName, String processedName) {
        ...
        if (ORE_LEVELS.containsKey(processedName) || ORE_LEVELS.containsKey(baseName))
            return "ore"; // Added baseName check
        if (BAR_LEVELS.containsKey(processedName) || BAR_LEVELS.containsKey(baseName))
            return "bar"; // Added baseName check
        if (HERB_LEVELS.containsKey(processedName))
            return processedName.startsWith("grimy") ? "herb-grimy" : "herb-cleaned";
        ...
        if (processedName.contains("hide") || processedName.contains("leather") || processedName.contains("fleece"))
            return "hide"; // Expanded
    ...
    private static class BankSortItem {
        ...
        private static String determineItemTier(String baseName) {
            ...
            if (baseName.contains("crystal pickaxe") || baseName.contains("crystal axe") || baseName.contains("crystal bow") || baseName.contains("crystal halberd") || baseName.contains("crystal shield"))
                return "crystal";
