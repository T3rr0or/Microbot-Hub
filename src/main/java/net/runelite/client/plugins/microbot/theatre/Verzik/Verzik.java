...
import net.runelite.client.plugins.microbot.theatre.prayer.TheatrePrayerUtil;
...
public class Verzik extends Room
{
	...
	private static final int VERZIK_VITUR_8369 = 8369;
	private static final int VERZIK_VITUR_8370 = 8370;
	private static final int VERZIK_VITUR_8371 = 8371;
	private static final int VERZIK_VITUR_8372 = 8372;
	private static final int VERZIK_VITUR_8373 = 8373;
	private static final int VERZIK_VITUR_8374 = 8374;
	private static final int VERZIK_VITUR_8375 = 8375;
	...
	private final Set<Projectile> handledPhaseThreeProjectiles = java.util.Collections.newSetFromMap(new IdentityHashMap<>());
	...
	private void updateConfig()
	{
		...
		for (String s : config.weaponSet().split(","))
		{
			try
			{
	...
	public void onMenuEntryAdded(MenuEntryAdded entry)
	{
		...
		{
			if (entry.getTarget().contains("Nylocas Athanatos") && entry.getMenuEntry().getType() == MenuAction.NPC_SECOND_OPTION)
			{
	...
	public void onProjectileMoved(ProjectileMoved event)
	{
		...
		if (projectile.getId() == VERZIK_RANGE_BALL)
		{
			verzikRangeProjectiles.put(projectile, WorldPoint.fromLocal(client, event.getPosition()));
		}
		...
		if (projectile.getId() != VERZIK_P3_RANGE_PROJECTILE && projectile.getId() != VERZIK_P3_MAGE_PROJECTILE)
		{
			return;
		}

		if (handledPhaseThreeProjectiles.contains(projectile))
		{
			if (projectile.getRemainingCycles() <= 0)
			{
				handledPhaseThreeProjectiles.remove(projectile);
			}
			return;
		}

		handledPhaseThreeProjectiles.add(projectile);

		if (projectile.getInteracting() == client.getLocalPlayer())
		{
			upcomingAttackQueue.add(new TheatreUpcomingAttack(
				(projectile.getRemainingCycles() / 30),
				(projectile.getId() == VERZIK_P3_MAGE_PROJECTILE ? Prayer.PROTECT_FROM_MAGIC : Prayer.PROTECT_FROM_MISSILES)
			));
		}
	}
	...
	public void onGameTick(GameTick event)
	{
		if (verzikActive)
		{
			...
			{
				if (verzikNPC.getId() == VERZIK_VITUR_8372 || verzikNPC.getId() == 10833 || verzikNPC.getId() == 10850)
				{
