...
{
    public static GiantMoleState state = GiantMoleState.IDLE;
     ...
     */
    public void updateGiantMoleState(GiantMoleConfig config)
    {
        if (needBanking(config))
        {
            state = GiantMoleState.BANKING;
            return;
        }
        ...
        if (isInMoleTunnel())
        {
            if (isMoleDead())
            {
                state = GiantMoleState.IDLE;
            }
            else if (isMoleNearby())
            {
                state = GiantMoleState.COMBAT;
            }
            else
            {
                state = GiantMoleState.HUNTING;
            }
        }
        ...
        if (isInFalador())
        {
            ...
            {
                state = GiantMoleState.WALKING_TO_MOLE_HOLE;
                return;
            }

            if (checkedIfWorldOccupied && isWorldOccupied)
            {
                state = GiantMoleState.HOPPING;
                return;
            }

            if (checkedIfWorldOccupied)
            {
                state = GiantMoleState.ENTERING_MOLE_LAIR;
            }
            else
            {
                state = GiantMoleState.IDLE;
